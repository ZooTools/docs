...
<Tip>
  Important note: If you or your marketing team modify the template and add new
  variable, make sure to update your API call so the template should always get
  the right variables. In the future we may response with an error in the API if
  you don't pass all the variables. Our suggestion is to first make the change
  in the backend passing the extra variables, and then update the template on
  ZooTools.
</Tip>
